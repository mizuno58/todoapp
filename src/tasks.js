...
postTasks = async function (body) {
  ...
  try {
    ...
    // ここに SQL を記述する
    const sql =
      "INSERT INTO todo_app.t_task (task_name, deadline, category_id) VALUES (?,?,?);";
...
getTasks = async function () {
  ...
  try {
    ...
    // ここに SQL を記述する
    const sql =
      "SELECT t_task.id, t_task.category_id, m_category.category_name, t_task.task_name, t_task.deadline, t_task.task_status, t_task.updated_at, t_task.created_at FROM t_task LEFT JOIN m_category ON t_task.category_id = m_category.id ORDER BY t_task.deadline asc;";
